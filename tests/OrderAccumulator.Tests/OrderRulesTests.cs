...
public class OrderRulesTests
{
    ...
    public void Valid_order_passes()
        => Assert.Null(OrderRules.Validate("PETR4", Buy, 100m, 25.50m));
    ...
    public void All_allowed_symbols_pass(string symbol)
        => Assert.Null(OrderRules.Validate(symbol, Buy, 10m, 1m));

    [Fact]
    public void Unknown_symbol_is_rejected()
        => Assert.NotNull(OrderRules.Validate("ITUB4", Buy, 100m, 25m));

    [Fact]
    public void Invalid_side_is_rejected()
        => Assert.NotNull(OrderRules.Validate("PETR4", 'X', 100m, 25m));

    [Fact]
    public void Non_integer_quantity_is_rejected()
        => Assert.NotNull(OrderRules.Validate("PETR4", Buy, 100.5m, 25m));
    ...
    public void Quantity_out_of_range_is_rejected(int qty)
        => Assert.NotNull(OrderRules.Validate("PETR4", Buy, qty, 25m));

    [Fact]
    public void Max_valid_quantity_passes()
        => Assert.Null(OrderRules.Validate("PETR4", Buy, 99_999m, 25m));
    ...
    public void Price_out_of_range_is_rejected(double price)
        => Assert.NotNull(OrderRules.Validate("PETR4", Buy, 100m, (decimal)price));

    [Fact]
    public void Price_with_more_than_two_decimals_is_rejected()
        => Assert.NotNull(OrderRules.Validate("PETR4", Buy, 100m, 25.501m));

    [Fact]
    public void Max_valid_price_passes()
        => Assert.Null(OrderRules.Validate("PETR4", Buy, 100m, 999.99m));
}
